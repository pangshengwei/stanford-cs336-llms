"""
Benchmark training speed with and without optimizer state sharding.

Compares iteration time for standard configuration (1 node, 2 GPUs, XL model).

Usage:
    python benchmark_sharded_optimizer.py --backend nccl --device cuda --world-size 2
"""
import argparse
import os
import sys
...
class XLModel(nn.Module):
    """XL transformer model for benchmarking (similar to assignment config)."""
    def __init__(self, d_model=1024, n_layers=24, vocab_size=50257):
        super().__init__()
...
def benchmark_optimizer(rank, world_size, backend, use_sharded, num_iterations, batch_size, seq_length):
    ...
    # Wrap with DDP
    ddp_model = nn.parallel.DistributedDataParallel(model, device_ids=[device.index] if backend == "nccl" else None)
...
def run_benchmark(rank, world_size, backend, args):
    ...
    if rank == 0:
        ...
        if overhead > 0:
            print(f"\n  Sharded optimizer is {overhead:.1f}% slower due to serial broadcasts")
        else:
            print(f"\n  Sharded optimizer has {abs(overhead):.1f}% lower latency")
...
def main():
    ...
    print(f"  Model: XL (1024 dim, 24 layers)")
