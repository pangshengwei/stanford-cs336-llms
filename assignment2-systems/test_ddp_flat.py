"""
Test script for flattened DDP implementation.

This script verifies the correctness of the flattened DDP implementation by comparing
...
Usage:
    ...
    python test_ddp_flat.py --backend nccl --device cuda --world-size 2
"""
import argparse
import os
import sys
...
def test_ddp_training(rank, world_size, backend, num_iterations=5):
    """
    Test flattened DDP training by comparing with single-process baseline.
    """
    ...
    ddp_model = FlattenedDDP(ddp_base)
    ...
    if rank == 0:
        ...
        if all_match:
            print(f"\n✓ SUCCESS: Flattened DDP weights match baseline (max_diff={max_diff:.2e})")
        else:
            print(f"\n✗ FAILURE: Flattened DDP weights do not match baseline (max_diff={max_diff:.2e})")
...
def main():
    parser = argparse.ArgumentParser(description="Test flattened DDP implementation")
    parser.add_argument(
        "--backend",
        type=str,
        default="gloo",
    ...
    print(f"Testing flattened DDP implementation:")
